...
class FAN():
    ...
    The FAN network
    '''
    def __init__(self, FLAGS=FLAGS, num_modules=1):
        super(FAN, self).__init__()
        self.num_modules = num_modules
        self.FLAGS = FLAGS

        mat_content = sio.loadmat(self.FLAGS.facemodel_path)
        self.DM = mat_content['DM68'][0,0]
        # Base part
        with tf.compat.v1.variable_scope("FAN"):
            ...
            for hg_module in range(self.num_modules):
                with tf.compat.v1.variable_scope("hg_module_%d"%hg_module):
                    ...
                    self._modules['l' + str(hg_module)] = layers.Conv2D(filters=self.FLAGS.num_lmks, 
                                                            kernel_size=1, strides=1, padding='same', 
        ...
        # inv part
        with tf.compat.v1.variable_scope("inv"):
            self.dense1 = layers.Dense(units=10, activation="relu", 
                            bias_initializer='zeros', name="inv_dense1")
            self.dense2 = layers.Dense(units=10, activation="relu", 
                            bias_initializer='zeros', name="inv_dense2")
            self.dense3 = layers.Dense(units=3,
                            bias_initializer='zeros', name="inv_dense3")
            self.conv1_inv = layers.Conv2D(filters=256, kernel_size=3, strides=1, activation="relu", 
                                        padding='same', name='inv_conv1')
            self.conv2_inv = layers.Conv2D(filters=self.FLAGS.num_lmks, kernel_size=3, strides=1, activation="relu", 
                                        padding='same', name='inv_conv2')

        with tf.compat.v1.variable_scope("pairwise"):     
            # C(C+1)/2 3
            self.lij_ = tf.Variable(initial_value=0.01*tf.stack([tf.ones([self.FLAGS.num_lmks*(self.FLAGS.num_lmks+1)/2]),
                                        tf.zeros([self.FLAGS.num_lmks*(self.FLAGS.num_lmks+1)/2]),
                                        tf.ones([self.FLAGS.num_lmks*(self.FLAGS.num_lmks+1)/2])], axis=1), 
                            trainable=True,
                            name='cij/l_fac')
            self.cij, self.inv_cov_pair = self.create_cij()
            C_IJ = mask_upper_tri_IJ(self.cij) # 2 2 m
    ...
    def compute_mean(self, outputs):
        '''
        Compute mean from the probability map
        '''
        self.output_mean = []
        self.output_cov_wh = []
    ...
    def M_from_y(self, pts2d, y3d_N, DM=DM):    
        '''
        Compute rotation from landmarks
        '''    
        # % pts2d: N C 2
        # % cij: 2 2 C C
        # % C_IJ_: 2m 2m
        ...
        # y_IJ_post: N 2 m
        # y_IJ_3d_post: N 3 m
        if self.FLAGS.facemodel_path is not None:
            DM = self.DM

        # C_IJ_ = self.C_IJ_
        C_IJ_ = tf.linalg.diag(tf.ones([tf.shape(self.C_IJ_)[0]]))
        # ----- % get y_IJ_post -----
        y = tf.transpose(pts2d, perm=[0,2,1]) # N 2 C            
        # N 2 C C # N 2 i j
        yij = tf.tile(tf.expand_dims(y, -1), [1,1,1,self.FLAGS.num_lmks]) - \
                tf.tile(tf.expand_dims(y, 2), [1,1,self.FLAGS.num_lmks,1])  # N 2 1 C
        # N 2 m
        y_IJ_post = tf.cast(mask_upper_tri_IJ(yij), dtype=tf.float32)

        #----- % get y_IJ_3d_post -----
        # N 3 C C # N 3 i j
        y_IJ_3d = tf.tile(tf.expand_dims(y3d_N, -1), [1,1,1,self.FLAGS.num_lmks]) - \
                tf.tile(tf.expand_dims(y3d_N, 2), [1,1,self.FLAGS.num_lmks,1])  # N 3 1 C
        # N 3 m
        ...
        M = tf.linalg.solve(mat, rhs) # N 6 1
    ...
    def q_from_y(self, pts2d, y_IJ_3d_post, M, y_IJ_post, DM=DM):
        '''
        Compute deformable model coefficient from landmarks
        '''    
        # M: N 2 3
        # y_IJ_3d_post: N 3 m
        # y_IJ_post: N 2 m
        ...
        phi_ij = tf.tile(tf.expand_dims(phi, -1), [1,1,1,self.FLAGS.num_lmks]) - \
                tf.tile(tf.expand_dims(phi, 2), [1,1,self.FLAGS.num_lmks,1]) # 3 numcomp C C
        ...
        # with tf.device('/CPU:0'):
        q = tf.linalg.solve(mat_q, rhs_q) # N numcomp 1

        return q


    def zeta_iterative(self, pts2d, DM=DM):
        '''
        Iteratively update PDM parameters
        '''    
        if self.FLAGS.facemodel_path is not None:
            DM = self.DM
        with tf.compat.v1.variable_scope("pairwise"):
            ...
            M = self.M_from_y(pts2d, y3d_N) # N 2 3
            
            for it in range(5):

                q = self.q_from_y(pts2d, y_IJ_3d_post, M, y_IJ_post) # N numcomp 1
                ...
                M = self.M_from_y(pts2d, pts3d_re) # N 2 3

            pts2d_re = tf.matmul(M, pts3d_re) # N 2 C

            # N 2 C C # N 2 i j
            self.muij = tf.tile(tf.expand_dims(pts2d_re, -1), [1,1,1,self.FLAGS.num_lmks]) - \
                    tf.tile(tf.expand_dims(pts2d_re, 2), [1,1,self.FLAGS.num_lmks,1])  # N 2 1 C
    ...
    def create_cij(self):
        '''
        Create raw CRF parameters
        '''    
        with tf.compat.v1.variable_scope("pairwise"):
            
            # C(C+1)/2 2 2
            lij = tf.contrib.distributions.fill_triangular(self.lij_) # lower
            lij_diag = tf.nn.relu(tf.linalg.diag_part(lij)) + epsilon # diagonal element >0
            self.lij = tf.linalg.set_diag(lij, lij_diag)
            ...
            cij = tf.linalg.set_diag(cij, tf.zeros([2,2,self.FLAGS.num_lmks]))
            cij = cij + tf.transpose(cij, perm=[0,1,3,2])
            
            # pairwise precision matrix
            sumcij_diag = tf.reduce_sum(cij, axis=-1) # 2 2 C
            inv_cov_pair = - cij
            inv_cov_pair = tf.linalg.set_diag(inv_cov_pair, sumcij_diag)
        return cij, inv_cov_pair        

        
    def crf_joint(self, mu, inv_cov, muij):
        '''
        ...
        # size
        cij = self.cij
        inv_cov_pair = self.inv_cov_pair
        N = tf.shape(mu)[0]
        C = tf.shape(mu)[1]
        size_cov = [N, 2*C, 2*C]

        # N 2 2 C C
        inv_cov_unary = tf.linalg.diag(tf.transpose(inv_cov, perm=[0,2,3,1]))
        ...
        ## cholesky_solve
        chol = tf.linalg.cholesky(precision_)
        mean_ = tf.linalg.cholesky_solve(chol, b_)

        return mean_, chol, precision_
    ...
    def loss_joint(self, mean_, labels, chol, precision_):
        ...
        loss = tf.reduce_mean(tf.matmul(tf.matmul(y_diff, precision_, transpose_a=True), y_diff) - \
                2*tf.reduce_sum(tf.math.log(tf.linalg.diag_part(chol)), -1))

        return loss


    def joint_iterative(self, mean, inv_cov, labels, joint_mean):
        '''
        Iterative Joint inference for CRF given unary mean and current joint mean
        '''
        for it in range(5):
            muij_, pts2d_re_, M_ = self.zeta_iterative(pts2d=tf.reshape(joint_mean, 
                                                    tf.shape(mean)))
            joint_mean, chol, precision_ = self.crf_joint(mean, inv_cov, muij_)

            loss = self.loss_joint(joint_mean, labels, chol, precision_)

            tf.add_to_collection('losses_joint', loss)

        return tf.add_n(tf.get_collection('losses_joint'), name='total_loss_joint'), joint_mean
    ...
    def learn_inv_cov(self, var_scope='inv'):
        ...
        for i in range(self.num_modules):
            ...
            h_8 = tf.reshape(h_8, shape=[tf.shape(h_8)[0], self.FLAGS.num_lmks, 64])
            h_8 = self.dense1(h_8)
            h_8 = self.dense2(h_8)
            h_8 = self.dense3(h_8)
    ...
    def crf_forward(self, images, labels, is_train=True):
        '''
        Forward pass through CNN-CRF, training process=True
        Unary and joint loss
        '''
        ...
        return self.loss_joint, self.joint_mean, mean, self.precision


    def crf_forward_predict(self, images, is_train=False):
        '''
        Forward pass through CNN-CRF, training process=False
        Used for prediction
        '''
        logits = self.forward(images, is_train=is_train)
        mean = self.compute_mean(logits)
        ...
        for it in range(10):
            muij_, pts2d_re_, M_ = self.zeta_iterative(pts2d=tf.reshape(joint_mean, 
                                                    tf.shape(mean[0])))
            joint_mean, _, precision = self.crf_joint(mean[-1], inv_cov[-1], muij_)
    ...
    def loss_total_crf_unary(self, output, labels, mean, inv_cov, logdet_invcov):
        ...
        for i in range(len(output)):
            ...
            L1_loss = 1*metric.L1_mean_loss(mean=mean[i], labels=labels)
